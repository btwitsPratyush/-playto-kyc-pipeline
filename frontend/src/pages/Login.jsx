      ...
      <div className="w-full max-w-[480px] bg-white p-10 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
        ...
        <div className="mt-8 pt-8 border-t border-slate-100">
          ...
          <div className="grid grid-cols-3 gap-2">
            <button 
              type="button"
              onClick={() => { setUsername('merchant_draft'); setPassword('password123'); }} 
              className="px-2 py-2 rounded-lg text-[11px] font-semibold text-slate-600 bg-slate-50 hover:bg-slate-100 border border-slate-200 transition-colors"
            >
              Draft
            </button>
            <button 
              type="button"
              onClick={() => { setUsername('merchant_review'); setPassword('password123'); }} 
              className="px-2 py-2 rounded-lg text-[11px] font-semibold text-slate-600 bg-slate-50 hover:bg-slate-100 border border-slate-200 transition-colors"
            >
              Review
            </button>
            <button 
              type="button"
              onClick={() => { setUsername('reviewer_1'); setPassword('password123'); }} 
              className="px-2 py-2 rounded-lg text-[11px] font-semibold text-slate-600 bg-slate-50 hover:bg-slate-100 border border-slate-200 transition-colors"
            >
              Reviewer
            </button>
